...
BINDINGS="${GOPATH}/src/github.com/wamuir/graft"
GOTESTPATH="${GOPATH}/src/github.com/wamuir/graft_package/libtensorflow_graft_test"
install -D -m400  "${GOTESTFILE}" "${GOTESTPATH}/main.go"
(
  cd "${GOTESTPATH}" \
  && ${GO} mod init libtensorflow_go \
  && ${GO} mod edit -require github.com/wamuir/graft@v0.0.0 \
  && ${GO} mod edit -replace github.com/wamuir/graft="${BINDINGS}" \
  && ${GO} mod tidy \
